...
function IntegrationsContainer() {
  ...
  return (
    <QueryCell
      query={query}
      success={({ data }) => {
        const conferencingItems = JSON.parse(JSON.stringify(data.conferencing.items)) || [];
        const calendar = data.calendar.items || [];
        const googleMeet = data.calendar.items.find(item => {
          return item.installed && item.type === "google_calendar";
        });
        if (googleMeet) {
          conferencingItems.push({...googleMeet,
            "imageSrc": "integrations/google-meet.svg",
            "title": "Google Meet",
            "description": "Video Conferencing",
          });
        }
        // console.log({conferencingItems, conferencing: data.conferencing.items})
        console.log({conferencingItems, calendar, googleMeet})
