...
export default function VerifyEmail({ csrfToken }) {
  const { t } = useLocale();
  ...
  const Success = () => {
    return (
      <div className="space-y-6">
        ...
        Your Email has successfully been verified, Click below button to login it to the app.
  ...
  return (
    <div className="flex flex-col justify-center min-h-screen py-12 bg-gray-50 sm:px-6 lg:px-8">
      <HeadSeo title={t("forgot_password")} description={t("verify_email")} />
          ...
          {!success && (
            <>
              <div className="space-y-6">
                ...
                <p>You must verify your eamil address before continuing.</p>
                {error && <p className="text-red-600">{error.message}</p>}
              </div>
              <form className="space-y-6" onSubmit={handleSubmit} action="#">
                ...
                <div>
                  <button
                    type="submit"
                    disabled={loading}
                    className={`w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-hiboxBlue-900 hover:bg-hiboxBlue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black ${
